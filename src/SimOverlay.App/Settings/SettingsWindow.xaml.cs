...
/// <summary>
/// Settings window — lazy singleton; call <see cref="OpenOrActivate"/> to show.
/// Sidebar selects which panel is shown in the content area.
/// Apply button → <see cref="OverlayManager.ApplyConfig"/> (saves to disk).
/// LostFocus on any panel field → <see cref="OverlayManager.PreviewConfig"/> (no save).
/// </summary>
public partial class SettingsWindow : Window
{
    ...
    // ── Nav items (bound to OverlayNavList) ───────────────────────────────────
    ...
    {
        // Intercept close (×) to hide instead of destroy — preserves state for next open.
        e.Cancel = true;
        Hide();
    }
}

...
public abstract class BaseOverlay : OverlayWindow
{
                   ...
                   : config)
    {
        ...
        Subscribe<SimStateChangedEvent>(e =>
        {
            ...
            // catch up вЂ” we must not call BringToFront() on every heartbeat or it fires
    ...
    // -------------------------------------------------------------------------
    // Config update вЂ” deferred invalidation (TASK-303)
    // -------------------------------------------------------------------------
    ...
    private void DrawEditDecoration(ID2D1RenderTarget context, float w, float h)
    {
        ...
        // Three diagonal dots in the bottom-right corner вЂ” indicate resize grip.
    ...
    // -------------------------------------------------------------------------
    // Window move / resize вЂ” keep config in sync (TASK-302)
    // -------------------------------------------------------------------------
    ...
    private void RenderLoop()
    {
        ...
        while (_running)
        {
            ...
            if (now >= nextFrameMs)
            {
                ...
                {
                    AppLog.Warn($"Device lost in '{DisplayName}' вЂ” recovering");
                    try
                    {
                        RecoverDevice();
                        OnDeviceRecovered();

...
public sealed class ZOrderHook : IDisposable
{
    ...
    public ZOrderHook(Action onTopmostReorder, IReadOnlyList<nint> ownedHandles)
    {
        _proc = (_, _, hwnd, idObject, _, _, _) =>
        {
            ...
            // Only react when the window that changed is also TOPMOST вЂ” avoids
            ...
            AppLog.Info($"ZOrderHook fired: hwnd=0x{hwnd:X} title='{sb}' class='{cls}' pid={pid} вЂ” calling BringAllToFront");
        ...
        if (_hook == nint.Zero)
            AppLog.Warn("ZOrderHook: SetWinEventHook returned null вЂ” z-order re-assertion disabled.");
        else

...
internal sealed class LmuMemoryReader : IDisposable
{
    ...
    /// look implausible вЂ” a cheap guard against LMU updates shifting the struct layout.
    ...
    private unsafe void ValidateLayout()
    {
        ...
        try
        {
            ...
            {
                AppLog.Info(
                    $"LmuMemoryReader: layout anchor check вЂ” NumVehicles={numVeh} (ok={numVehOk}), " +
            ...
            {
                AppLog.Info(
                    $"LmuMemoryReader: layout anchors OK вЂ” NumVehicles={numVeh}, CurrentET={currentEt:F1}s.");
            }
        }
        catch (Exception ex)
        {
    ...
    public unsafe LmuPlayerInputs? ReadPlayerTelemetry()
    {
        ...
        try
        {
            ...
            //   [+1] uint8_t playerVehicleIdx   в†ђ authoritative telemetry array index

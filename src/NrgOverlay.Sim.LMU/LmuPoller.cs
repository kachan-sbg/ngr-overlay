...
///   <item><see cref="SessionData"/> вЂ” on session change (track / vehicle count / session)</item>
/// </list>
/// </summary>
internal sealed class LmuPoller : IDisposable
{
    private const int RelativePublishInterval = 6;  // 60 Hz Г· 6 = 10 Hz
    private const int WeatherPublishInterval  = 60; // 60 Hz Г· 60 = 1 Hz
    ...
    public void Start()
    {
        ...
        AppLog.Info("LmuPoller started вЂ” will open LMU_Data on first successful poll.");
    }
    ...
    private void Poll(object? state)
    {
        ...
        try
        {
            ...
            // Always evaluate in-session state each tick вЂ” InRealtime can flip
            // 0в†’1 when the race starts without any TrackName/Session/NumVehicles
            ...
            // into the telemetry header вЂ” use that instead of the scoring array index.
    ...
    // в”Ђв”Ђ Session handling в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    private void HandleSessionChange(LmuScoringSnapshot snapshot)
    {
        ...
        AppLog.Info(
            $"LmuPoller: session info updated вЂ” track={info.TrackName}, " +
    ...
    /// <c>InRealtime</c> can flip 0в†’1 (race start) without any change to
    ...
    private void UpdateSessionState(LmuScoringInfo info)
    {
        ...
        if (_inSession)
        {
            ...
            AppLog.Info($"LmuPoller: InRealtimeв†’1 вЂ” session active ({info.TrackName})");
        }
        else
        {
            _onStateChanged(SimState.Connected);
            AppLog.Info("LmuPoller: InRealtimeв†’0 вЂ” session paused/ended.");
        }
    }

...
internal sealed class FuelConsumptionTracker
{
    ...
    {
        // Track caution state вЂ” any tick under caution taints the whole lap.
        if ((sessionFlags & CautionMask) != 0)
            _cautionThisLap = true;

        if (_lastLap < 0)
        {
            // First tick вЂ” initialise without recording consumption.
            _lastLap        = lap;
    ...
    /// <summary>Resets all state вЂ” call on sim disconnect / session change.</summary>
    public void Reset()
    {
        _buffer.Clear();
        _lastLap           = -1;

...
**Decision:** Each `OverlayWindow` creates a hidden 0×0 `WS_EX_TOOLWINDOW` owner HWND and passes it as `hWndParent` to the overlay `CreateWindowEx` call. The owner is destroyed after the overlay in `Dispose`.

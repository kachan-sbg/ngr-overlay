...
| 04-04 | Omit WS_EX_LAYERED *(superseded 04-05)* | DComp alpha hit-test caching broke resize вЂ” later moot after ULW switch |
...
| 04-07 | Upgrade IRSDKSharper 1.0.3в†’1.1.6; deterministic Win32 handle release on disconnect | 1.0.3 left the MMF handle open after `Stop()`, causing "pending" state on sim restart |
...
| 04-07 | SimDetector provider list sorted by `SimPriorityOrder` config at startup; config v3в†’v4 appends "LMU" | User-adjustable priority without code change; migration preserves existing order |

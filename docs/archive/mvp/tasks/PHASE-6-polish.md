...
- **Description**: In `Program.cs`, acquire named `Mutex` (`Global\NrgOverlay_SingleInstance`) before showing UI. If already held: find existing instance's tray window, post message to bring Settings to foreground, exit. Use `CreateMutex` via P/Invoke.
...
- **Description**: Create `.ico` file (16Г—16, 32Г—32, 48Г—48, 256Г—256). Set as assembly icon in `NrgOverlay.App.csproj`. Use for `NotifyIcon`. Add `Resources/` folder in `App`.
...
- **Fix**: Each `OverlayWindow` now creates a hidden 0Г—0 `WS_EX_TOOLWINDOW` owner HWND and passes it as `hWndParent`. Owned windows don't receive taskbar buttons. OBS visibility unaffected (WGC enumerates owned HWNDs normally).
...
- **Change**: `ShowInTaskbar` changed from `False` to `True`; icon loaded from `Resources/nrgoverlay.ico` at startup. X button already hid to tray (`Window_Closing` cancels + `Hide()`) вЂ” no logic change needed.
- **Fixed in:** Phase 6 commit (2026-04-05)
...
- **Note (2026-04-06)**: Benchmark project created and first run completed on AMD Ryzen 5 7535HS / .NET 8.0.25. SimDataBus results: Publish1Subscriber = 9.3 ns / 0 B alloc; Publish3Subscribers = 13.6 ns / 0 B alloc; PublishNoSubscribers = 5.1 ns / 0 B alloc вЂ” all zero allocation, all well under the 1 Вµs target. Full results (RelativeCalculator + ConfigResolve) in `BenchmarkDotNet.Artifacts/results/`. Manual profiling criteria (CPU < 3%, GPU < 50 MB, frame time variance < 2 ms p99) remain valid real-session targets but require a live sim вЂ” out of scope for this automated suite.

---

**TASK-606** `[x]`
- **Title**: Installer / distribution package
- **Description**: Create a self-contained publish profile (`dotnet publish -r win-x64 --self-contained -p:PublishSingleFile=true`). Optionally create a NSIS/WiX installer placing the exe in `%ProgramFiles%\NrgOverlay\`, with a Start Menu shortcut and "Start with Windows" option. Include `README.md` and `CHANGELOG.md`.
...
- **Note (2026-04-05)**: Self-contained single-file publish profile created at `src/NrgOverlay.App/Properties/PublishProfiles/win-x64.pubxml`. NSIS/WiX installer deferred вЂ” out of scope for MVP. Publish command: `dotnet publish src/NrgOverlay.App -p:PublishProfile=win-x64`.

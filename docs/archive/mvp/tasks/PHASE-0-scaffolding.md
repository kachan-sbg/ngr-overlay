# Phase 0 вЂ” Project Scaffolding `[x]`
...
- **Acceptance Criteria**: `dotnet build NrgOverlay.sln` completes with no errors. All project references are correct and no circular dependencies exist. The solution loads in Visual Studio 2022 without errors.
- **Dependencies**: None.
...
- **Description**: Add NuGet packages to appropriate projects. `NrgOverlay.Rendering`: `Vortice.Direct2D1`, `Vortice.DirectComposition`, `Vortice.DXGI`, `Vortice.Direct3D11`. `NrgOverlay.Sim.iRacing`: `IRSDKSharper`. `NrgOverlay.App`: `Microsoft.Extensions.DependencyInjection`, `Microsoft.Extensions.Hosting`. `NrgOverlay.Core`: `System.Text.Json` (already in .NET 8 BCL, no explicit package needed). Create `Directory.Packages.props` for central package version management.

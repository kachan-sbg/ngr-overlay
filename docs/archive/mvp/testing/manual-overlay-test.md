# Manual Overlay Smoke Test вЂ” TASK-405
...
1. Build `NrgOverlay.sln` (Release or Debug, x64).
2. Delete `%APPDATA%\NrgOverlay\config.json` if one exists from a previous run (ensures defaults are used).
3. Launch `NrgOverlay.App.exe`.
...
### C вЂ” Session Info overlay

- [ ] Track name shown at top (e.g., "Lime Rock Park").
- [ ] Session type shown (e.g., "Practice В· MM:SS remaining" or "Practice В· HH:MM:SS remaining").
...
- [ ] Air and Track temps show numeric values with В°C unit.
...
- [ ] Trend arrow (в–І/в–ј) appears after ~30 frames of data and changes direction.

---

### E вЂ” Edit mode (drag and resize)

- [ ] Right-click tray icon в†’ "Edit Overlays" (or equivalent) enters edit mode.

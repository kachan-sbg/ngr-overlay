...
### TASK-001 — Solution and project structure

```
dotnet build SimOverlay.sln
```
- [ ] Build exits 0 with no errors
- [ ] No circular dependency errors
- [ ] Solution opens in Visual Studio 2022 / VS Code without missing projects

### TASK-002 — NuGet packages

```
dotnet restore SimOverlay.sln --force
dotnet build SimOverlay.sln
```
- [ ] Restore exits 0 with no warnings (especially NU1603 version approximation warnings)
- [ ] Build exits 0

### TASK-003 — Global build properties and CI

```
dotnet build SimOverlay.sln -warnaserror
```
...
```
dotnet test tests/SimOverlay.Core.Tests/SimOverlay.Core.Tests.csproj
```
...
### TASK-102 — ConfigStore and config types
...
### TASK-103 — Sim.Contracts DTOs and ISimProvider
...
### TASK-104 — Win32 transparent overlay window

Run the app manually:
```
dotnet run --project src/SimOverlay.App
```
...
- [ ] Window title is set (e.g., "SimOverlay — Relative") — verify with Spy++ or Task Manager
...
### TASK-105 — DXGI swap chain and Direct2D setup
...
### TASK-106 — BaseOverlay render loop

```
dotnet test tests/SimOverlay.Overlays.Tests/SimOverlay.Overlays.Tests.csproj
```
- [ ] `TestOverlay` (red rectangle) renders at ~60 fps — verify with frame counter or stopwatch
...
### TASK-107 — Lock/unlock edit mode
...
### TASK-201 — IRacingProvider connection lifecycle
...
### TASK-202 — IRacingPoller 60 Hz loop

With iRacing in a session:
- [ ] `DriverData` is published at 58–62 Hz (measure over 5 seconds)
- [ ] `RelativeData` is published at 9–11 Hz
- [ ] No memory growth after 10 minutes
- [ ] Polling thread CPU usage stays below 2% on a modern machine

### TASK-203 — Session YAML decoder
...
- [ ] `SessionData` republishes on session change (practice → qualify transition)

### TASK-204 — Relative gap calculator

```
dotnet test tests/SimOverlay.Sim.iRacing.Tests/SimOverlay.Sim.iRacing.Tests.csproj
```
- [ ] Gap is correct for a car directly ahead
- [ ] Gap is correct for a car directly behind
- [ ] Wrap-around at S/F line produces a near-zero gap (not ±1 lap)
- [ ] Player entry is always present and marked `IsPlayer = true`
- [ ] Output list is sorted by gap (most ahead at index 0)

### TASK-205 — iRacing integration test

With iRacing running in a session:
```
dotnet test tests/SimOverlay.Sim.iRacing.Tests/ --filter Category=Integration
```
- [ ] At least 280 `DriverData` messages in 5 seconds (≥56 Hz)
- [ ] At least 45 `RelativeData` messages in 5 seconds (≥9 Hz)
...
### TASK-302 — Position/size persistence
...
### TASK-304 — Sim state display

- [ ] App started without iRacing running → overlays show "Sim not detected"
- [ ] Start iRacing to the main menu → overlays show "Waiting for session…"
- [ ] Enter a session → overlays show live data
- [ ] Close iRacing → overlays revert to "Sim not detected"

---

## Phase 4 — MVP Overlays

### TASK-401 — Relative overlay

With a live iRacing session (≥3 cars):
...
### TASK-402 — Relative column visibility
...
### TASK-403 — Session Info overlay

With a live iRacing session:
- [ ] Track name, session type, time remaining are correct
- [ ] Track/air temps correct and unit-configurable (°C / °F)
...
- [ ] Bar is clamped at the edge for large deltas — does not overflow the window
- [ ] Trend arrow shows correct direction
- [ ] Smooth visual update at 60 Hz (no stuttering)

### TASK-405 — Overlay smoke test
...
- [ ] Window opens from tray icon → "Settings"
...
### TASK-603 — Application icon

- [ ] Icon appears in Explorer when browsing to the `.exe`
- [ ] Icon appears in the tray notification area

### TASK-604 — Error logging

- [ ] Log file created at `%APPDATA%\SimOverlay\sim-overlay.log`
...
```
dotnet publish src/SimOverlay.App -r win-x64 --self-contained -p:PublishSingleFile=true
```
- [ ] Single `.exe` produced

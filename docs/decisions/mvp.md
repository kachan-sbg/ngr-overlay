...
## 2026-04-04 — Omit `WS_EX_LAYERED` from overlay window styles *(superseded 2026-04-05)*
...
**Context:** During Phase 1 implementation, overlay windows were initially created with `WS_EX_LAYERED`. After adding resize support, resizing the window beyond its original size stopped generating mouse events in the expanded area — the interactive region stayed permanently at the original 300x100 pixels.
...
## 2026-04-05 — Single-instance enforcement via hidden HWND + named Mutex

**Decision:** `SingleInstanceGuard` acquires `Global\SimOverlay_SingleInstance` mutex. If already held (second instance), it finds the first instance's hidden message-only HWND (`SimOverlay_Cmd`) using `FindWindow`, posts `WM_APP`, then exits. The first instance's `WndProc` fires `OpenSettingsRequested` which opens the Settings window.
...
**Rationale:**
- A named Mutex is the idiomatic Win32 single-instance mechanism — reliable across sessions and users.
...
- The hidden HWND is created on the STA thread that runs the Win32 message pump, so `WndProc` dispatches naturally via `DispatchMessage` — no additional thread needed.
...
- Class name `SimOverlay_Cmd` is a fixed contract between instances. Do not rename without updating both send and receive sides.

---

## 2026-04-05 — App icon: pre-build generator tool + runtime ICO load

**Decision:** App icon is generated by `tools/GenerateIcon` (a standalone .NET console app) which writes `src/SimOverlay.App/Resources/simoverlay.ico`. The csproj references this as `<ApplicationIcon>` and copies it to the output directory. `TrayIconController` loads it at runtime from the output `Resources/` subfolder, falling back to a programmatic icon if the file is absent.
...
**Rationale:**
- The generator uses `System.Drawing` to render the icon at each required size and serialises PNG chunks into a valid ICO container — no external tooling required beyond the .NET SDK.
...
- The generated icon is a placeholder with a simple "S" letterform — replace `Resources/simoverlay.ico` with a designer-produced asset at any time without code changes.

---

## 2026-04-05 — Hidden owner HWND suppresses overlay taskbar buttons while preserving OBS visibility
...
- OBS WGC enumerates windows via `EnumWindows`, which returns all top-level HWNDs regardless of owner relationship — owned windows are still top-level in Win32 terms.
...
- **`WS_EX_APPWINDOW` removal**: `WS_EX_APPWINDOW` forces a taskbar button even for owned windows, but it was never set — the issue was the absence of an owner, not the presence of APPWINDOW.
...
## 2026-04-05 — Settings window ShowInTaskbar changed to True; X hides to tray
...
## 2026-04-05 — BenchmarkDotNet for automated performance regression detection

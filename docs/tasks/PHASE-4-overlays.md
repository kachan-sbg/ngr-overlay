...
- **Note (2026-04-05)**: Smoke test run against live iRacing session. Data renders and updates correctly across all three overlays. One known visual glitch found and deferred: brief ~1-frame blink on all overlays every ~2 seconds caused by `SimDetector` reacting to transient `IsRunning() == false` — logged as ISSUE-014.

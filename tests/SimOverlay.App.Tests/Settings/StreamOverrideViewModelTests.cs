...
public class StreamOverrideViewModelTests
{
    ...
    // ── LoadFrom with null StreamOverrideConfig ───────────────────────────────
    ...
    // ── LoadFrom with partial StreamOverrideConfig ────────────────────────────
    ...
    public void LoadFrom_PartialOverride_HasXxxOnlyForSetFields()
    {
        var src = new StreamOverrideConfig
        {
            ...
            Width   = 800,         // set
            FontSize = 20f,        // set
            // Height, Opacity, etc. → null (not set)
        };
    ...
    public void ToConfig_WhenEnabled_OnlyIncludesHasXxxFields()
    {
        ...
        Assert.Null(result.Height);    // not set → must remain null
        Assert.Null(result.FontSize);  // not set → must remain null
        Assert.Null(result.Opacity);   // not set → must remain null
    }
    ...
    public void LoadFrom_ToConfig_RoundTrip_PreservesEnabledAndSetFields()
    {
        ...
        Assert.Null(result.Height);   // was null in src → must still be null
        Assert.Null(result.Opacity);  // was null in src → must still be null
    }
}

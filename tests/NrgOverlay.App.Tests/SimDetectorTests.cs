...
namespace NrgOverlay.App.Tests;
...
///   <item>First-connect wins вЂ” no switching while the active provider is running.</item>
///   <item>Disconnect debounce вЂ” <see cref="SimDetector.DisconnectThreshold"/> consecutive
///     false results needed before deactivation.</item>
///   <item>Recovery вЂ” a Disconnecting provider that comes back returns to Active without
...
{
    // в”Ђв”Ђ Fakes в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    // Shortcut вЂ” reads the ProviderState for a named provider from the detector.
    private static ProviderState State(SimDetector det, string simId)
        => det.ProviderStates[simId];

    // в”Ђв”Ђ Baseline в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    public void Poll_SimAppears_TransitionsIdleToAvailableToActive()
    {
        ...
        p1.Running = true;
        det.Poll();
        // Available в†’ Active in the same tick (no other provider was active).
    ...
    // в”Ђв”Ђ First-connect-wins (no switching) в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    public void Poll_FirstConnectWins_NoSwitchWhenSecondSimAppears()
    {
        ...
        // iRacing starts вЂ” must NOT trigger a switch.
        pB.Running = true;
        det.Poll();
        det.Poll();
    ...
    // в”Ђв”Ђ Disconnect debounce в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    public void Poll_FirstFalse_GoesDisconnecting_NotStopped()
    {
        ...
        det.Poll();       // Active
        p1.Running = false;
        det.Poll();       // strike 1 в†’ Disconnecting
    ...
    {
        // DisconnectThreshold = 3: Active в†’ Disconnecting (strike 1) в†’ strike 2 в†’ strike 3 в†’ Idle.
        var bus = new MockBus();
        var p1  = new MockProvider("iRacing", running: true);
        using var det = Make(bus, [p1]);
        ...
        det.Poll();        // strike 1 в†’ Disconnecting
        det.Poll();        // strike 2
        det.Poll();        // strike 3 в†’ Idle, deactivated
    ...
    public void Poll_Disconnecting_RecoveryBeforeThreshold_ReturnsToActive()
    {
        ...
        det.Poll();        // strike 1 в†’ Disconnecting
        p1.Running = true;
        det.Poll();        // recovery в†’ Active (no restart)
    ...
    public void Poll_StrikesReset_AfterRecovery_RequiresFullThresholdAgain()
    {
        ...
        // Go to strike 2, recover, then go to strike 2 again вЂ” still should not stop.
        p1.Running = false;
        det.Poll();        // strike 1
        det.Poll();        // strike 2
        p1.Running = true;
        det.Poll();        // recovery вЂ” reset strikes
        p1.Running = false;
        ...
        det.Poll();        // strike 3 в†’ stop

        Assert.True(p1.Stopped);
    }

    // в”Ђв”Ђ Post-disconnect activation в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    public void Poll_AfterActiveDisconnects_SecondSimActivates_SameTick()
    {
        ...
        pA.Running = false;
        det.Poll();  // strike 1 в†’ pA Disconnecting
        det.Poll();  // strike 2
        det.Poll();  // strike 3 в†’ pA Idle, pB activates in same tick
    ...
    public void Poll_AfterActiveDisconnects_NewSimAppearingNextPollActivates()
    {
        ...
        det.Poll();        // p1 active
        p1.Running = false;
        det.Poll(); det.Poll(); det.Poll(); // three strikes в†’ p1 idle
    ...
    // в”Ђв”Ђ ProviderStates snapshot в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    // в”Ђв”Ђ Bus events в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    public void Poll_ActiveProviderChanged_FiresNullOnDeactivate()
    {
        ...
        det.Poll();             // activate
        p1.Running = false;
        det.Poll(); det.Poll(); det.Poll(); // three strikes в†’ deactivate
    ...
    // в”Ђв”Ђ Exception resilience в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    // в”Ђв”Ђ Disposal в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ

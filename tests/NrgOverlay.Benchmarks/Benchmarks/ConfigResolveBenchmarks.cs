...
/// <summary>
/// Benchmarks <see cref="OverlayConfig.Resolve"/> вЂ” called once per overlay per frame
/// (60 Hz Г— 3 overlays = 180 calls/sec).
///
/// Targets:
///   ResolveNoOverride   : 0 B alloc  (returns 'this' вЂ” no object created)
...
public class ConfigResolveBenchmarks
{
    ...
    public void Setup()
    {
        ...
        // Override present but disabled вЂ” should take the fast path (return this)
    ...
    /// <summary>No stream override вЂ” returns 'this', zero allocations.</summary>
    [Benchmark(Baseline = true)]
    public OverlayConfig ResolveNoOverride() => _noOverride.Resolve(streamModeActive: false);

    /// <summary>Stream mode active, override enabled вЂ” allocates one new OverlayConfig.</summary>
    [Benchmark]
    public OverlayConfig ResolveWithOverride() => _withOverride.Resolve(streamModeActive: true);

    /// <summary>Stream mode active but override disabled вЂ” takes fast path, zero allocations.</summary>
    [Benchmark]
